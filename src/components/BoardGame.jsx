    ...
    return (
        <>
            <div className="flex flex-col lg:flex-row gap-5 lg:gap-10 px-5 lg:px-20 py-10 lg:py-24">
                ...
                <motion.div className="w-full lg:w-1/2 flex flex-nowrap justify-center md:justify-start gap-3 order-2 lg:order-3" variants={cardVariants} initial="offscreen" whileInView="onscreen" viewport={{ once: true }}>
                    {imageUrl.map((url, index) => (
                        <img src={url} alt="prawara-board-game" className="rounded-md w-40 md:w-64 lg:w-[21.5rem] shadow-xl" key={index}  />
                    ))}
                </motion.div>
            </div>
            <motion.div className="flex flex-wrap gap-3 md:gap-5 w-full justify-center md:justify-start lg:justify-center md:pl-10 lg:pl-0" variants={cardVariants} initial="offscreen" whileInView="onscreen" viewport={{ once: true }}>
                {imgBoardGame.map((img, index) => (
                    <img src={img} alt="prawara-board-game" className="rounded-md w-[22rem] md:w-[25rem] lg:w-[28rem] shadow-xl" key={index} />
                ))}
            </motion.div>
        </>
    )
